...
MODIFIED to correctly handle relative paths in parameter stream files.
...
# ===============================================================
# MODIFIED FUNCTION
# This version correctly resolves paths relative to the stream file.
# ===============================================================
def read_params(filename):
    """
    Reads a CHARMM-GUI parameter stream file.
    
    It correctly resolves the paths of the parameter files listed inside
    the stream file, making them relative to the stream file's location.
    """
    # Get the directory where the main parameter stream file is located.
    base_dir = os.path.dirname(filename)
    if base_dir == '': # If the file is in the CWD, dirname is empty
        base_dir = '.'
        
    parFiles = ()
    for line in open(filename, 'r'):
        if '!' in line: line = line.split('!')[0]
        parfile_relative = line.strip()
        if len(parfile_relative) != 0:
            # Construct the full path by joining the base directory with the relative path
            full_parfile_path = os.path.join(base_dir, parfile_relative)
            parFiles += (full_parfile_path,)

    #print(f"Reading parameter files: {parFiles}")
