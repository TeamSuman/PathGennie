...
def read_top(filename, fftype='CHARMM', ffdir = None, gro = None):
    if filename.endswith('top'):
        gro = GromacsGroFile(gro)
        top = GromacsTopFile(filename, periodicBoxVectors = gro.getPeriodicBoxVectors(),
                        includeDir = ffdir)
    else:
...
def read_crd(filename, fftype='CHARMM'):
    if filename.endswith('.gro'):
        ...
        # elif filename.endswith('.gro'): crd = GromacsGroFile(filename)
    return crd
